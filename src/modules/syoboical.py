...
async def anime_prog(day):
    ...
    for ProgItem in root.iter('ProgItem'):
        ...
        if int(day) == 0:
            if re.compile('1|2|3|4|5|6|15|16|17|18|71|128').fullmatch(ProgItem.get('ChID')) and st[4:8] == day_o and st[4:12] >= day_m:
                ...
                if len(number) > 0:
                    ...
                    data = st[4:6] + '/' + st[6:8] + ' ' + st[8:10] + ':' + st[10:12] + subtitle + ' ' + chname
                anime_data.append([title, data])
        else:
            if re.compile('1|2|3|4|5|6|15|16|17|18|71|128').fullmatch(ProgItem.get('ChID')) and st[4:8] == day_o:
                chname = ProgItem.get('ChName')
                title = ProgItem.get('Title')
                subtitle = ProgItem.get('SubTitle')
                number = ProgItem.get('Count')
